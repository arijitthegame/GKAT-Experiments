...
def compute_lanczos_matrix_exp(
    ...
    T, Q = lanczos(
        ...
        num_eig_vec=k,
        mask=None,
        use_reorthogonalization=use_reorthogonalization,
    )

    D, P = torch.linalg.eigh(T)
    exp_T = torch.bmm(
        torch.bmm(P, torch.diag_embed(torch.exp(D))), P.transpose(1, 2)
    )
...
def compute_exprational_matrix_exp(
    A, v, k, return_exp=False, method_type="cg", tolerance=1e-6, eps=1e-15
):
    """
