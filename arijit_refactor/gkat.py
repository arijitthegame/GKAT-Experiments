...
class GKATLayer(nn.Module):
    ...
    def forward(
        ...
        logits = torch.matmul(Q, torch.transpose(K, 0, 1)) / math.sqrt(
            Q.shape[1]
        )  # no softmax
        ...
        self.layers = nn.ModuleList(
            [
                ...
                nn.ModuleList(
                    [
                        GKATLayer(
                            self.hidden_dim[0] * self.num_heads,
                            self.hidden_dim[-1],
                            feat_drop=self.feat_drop,
                            attn_drop=self.attn_drop,
                            agg_activation=self.agg_activation,
...
class GKATMultiHead(nn.Module):
    ...
    def forward(
        ...
        if counting_attn.dim() == 2:
            counting_attn = repeat(
                counting_attn, "l w -> h l w", h=self.num_heads
            )  # repeat along heads
            counting_attn = counting_attn.unsqueeze(0)  # add batch dim

        elif counting_attn.dim() == 3:
            counting_attn = repeat(
                counting_attn, "b l w -> b h l w", h=self.num_heads
            )  # repeat along heads
        ...
        logit_mask = torch.einsum(
            "b h i j, b h j k -> b h i k", torch.exp(logits), counting_attn
        )
        logit_mask = logit_mask / (
            torch.sum(logit_mask, 3, keepdim=True) + 1e-9
        )
...
class GKATMultiHeadClassifier(nn.Module):
    def __init__(
        ...
        self.final_gkat_layer = GKATMultiHead(
            self.hidden_dim[0],
            self.hidden_dim[-1],
            num_heads=1,
            feat_drop=self.feat_drop,
